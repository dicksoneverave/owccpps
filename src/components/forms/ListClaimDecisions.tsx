    ...
    const fetchAllCaseData = async (irn: number) => {
      ...
      const tables = await Promise.all([
        fetchAndFormat('timebarredclaimsregistrarreview', 'IRN, TBCRRFormType, TBCRRReviewStatus, TBCRRDecisionReason, TBCRRDecisionDate', row => ({
          IRN: row.IRN, submissionType: `${row.TBCRRFormType} - TimeBarred`, status: row.TBCRRReviewStatus, reason: row.TBCRRDecisionReason, takenBy: 'Registrar', decisionDate: row.TBCRRDecisionDate
        })),
        fetchAndFormat('prescreening_view', '*', row => ({
          IRN: row.IRN, submissionType: row.PRFormType, status: row.PRStatus, reason: row.PRDecisionReason, takenBy: 'Deputy Registrar', decisionDate: row.PRSubmissionDate
        })),
        fetchAndFormat('registrarreview', 'IRN, IncidentType, RRStatus, RRDecisionReason, RRDecisionDate', row => ({
          IRN: row.IRN, submissionType: row.IncidentType, status: row.RRStatus, reason: row.RRDecisionReason, takenBy: 'Registrar', decisionDate: row.RRDecisionDate
        })),
        fetchAndFormat('form6master', 'IRN, IncidentType, F6MStatus, F6MApprovalDate', row => ({
          IRN: row.IRN, submissionType: `${row.IncidentType} (Form6)`, status: `${row.F6MStatus} (Notification Received - Insurance Company)`, reason: '--', takenBy: '--', decisionDate: row.F6MApprovalDate
        })),
        fetchAndFormat('form18master', 'IRN, IncidentType, F18MStatus, F18MEmployerDecisionReason, F18MWorkerDecisionReason, F18MWorkerAcceptedDate', row => {
          ...
          return { IRN: row.IRN, submissionType: `${row.IncidentType} - Form18 Notification`, status: row.F18MStatus, reason, takenBy: by, decisionDate: row.F18MWorkerAcceptedDate }
        }),
     /*   fetchAndFormat('tribunalhearingschedule', 'IRN, THSHearingType, THSHearingStatus, THSDecisionDate', row => ({
        ...
        fetchAndFormat('approvedclaimscporeview', 'IRN, IncidentType, CPORStatus, LockedByCPOID, CPORApprovedDate', row => {
          ...
          return { IRN: row.IRN, submissionType: row.IncidentType, status, reason: '--', takenBy, decisionDate: row.CPORApprovedDate }
        }),
        fetchAndFormat('compensationcalculationcommissionersreview', 'IRN, IncidentType, CCCRReviewStatus, CCCRDecisionReason, CCCRDecisionDate', row => ({
          IRN: row.IRN, submissionType: row.IncidentType, status: row.CCCRReviewStatus, reason: row.CCCRDecisionReason, takenBy: row.CCCRReviewStatus?.startsWith('Chief') ? 'Chief Commissioner' : 'Commissioner', decisionDate: row.CCCRDecisionDate
        })),
        fetchAndFormat('claimsawardedcommissionersreview', 'IRN, IncidentType, CACRReviewStatus, CACRDecisionReason, CACRDecisionDate', row => ({
          IRN: row.IRN, submissionType: row.IncidentType, status: row.CACRReviewStatus, reason: row.CACRDecisionReason, takenBy: row.CACRReviewStatus?.startsWith('Chief') ? 'Chief Commissioner' : 'Commissioner', decisionDate: row.CACRDecisionDate
        })),
       fetchAndFormat('compensationcalculationcpmreview', 'IRN, IncidentType, CPMRStatus, CPMRDecisionReason, CPMRDecisionDate', row => ({
          IRN: row.IRN, submissionType: row.IncidentType, status: row.CPMRStatus, reason: row.CPMRDecisionReason, takenBy: 'CPM', decisionDate: row.CPMRDecisionDate
        })), 
        fetchAndFormat('claimsawardedregistrarreview', 'IRN, IncidentType, CARRReviewStatus, CARRDecisionReason, CARRDecisionDate', row => ({
          IRN: row.IRN, submissionType: row.IncidentType, status: row.CARRReviewStatus, reason: row.CARRDecisionReason, takenBy: 'Registrar', decisionDate: row.CARRDecisionDate
        })),
     /*   fetchAndFormat('compensationcalculationregistrarreview', 'IRN, IncidentType, CCRReviewStatus, CCRDecisionReason, CCRDecisionDate', row => ({
          IRN: row.IRN, submissionType: row.IncidentType, status: row.CCRReviewStatus, reason: row.CCRDecisionReason, takenBy: 'Registrar', decisionDate: row.CCRDecisionDate
        })),
